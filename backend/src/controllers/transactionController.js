...
async function listTransactions(req, res) {
  ...
  if (search) {
    params.push(`%${search}%`);
    // Fixed: users no longer have a single `name` column — search first/last name instead.
    conditions.push(`(u.first_name ILIKE $${params.length} OR u.last_name ILIKE $${params.length} OR t.payment_method ILIKE $${params.length})`);
  }
  ...
  try {
    const result = await pool.query(
      `SELECT t.*, (u.first_name || ' ' || u.last_name) AS cashier_name
       FROM transactions t
...
async function getTransaction(req, res) {
  ...
  try {
    const txResult = await pool.query(
      `SELECT t.*, (u.first_name || ' ' || u.last_name) AS cashier_name FROM transactions t
